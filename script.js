...
async function callGemini(question){
  ...
  const history=state.history.filter(m=>m.text!=="__LOADING__").slice(-10).map(m=>({role:m.role==="model"?"model":"user",parts:[{text:m.text}]}));
  const body={contents:[...history,{role:"user",parts:[{text:aiPrompt(question)}]}],generationConfig:{maxOutputTokens:8000}};
  if(state.webSearch) body.tools=[{google_search:{}}];
  const models=[state.model,"gemini-3.6-flash","gemini-3.5-flash","gemini-3.5-flash-lite"].filter((m,i,a)=>m&&a.indexOf(m)===i);
  let lastError="Gemini request failed.";
  for(const model of models){
    const url=`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent`;
    for(let attempt=0;attempt<3;attempt++){
      try{
        const response=await fetch(url,{method:"POST",headers:{"Content-Type":"application/json","x-goog-api-key":state.apiKey},body:JSON.stringify(body)});
        const data=await response.json().catch(()=>({}));
        if(response.ok){
          const text=(data?.candidates?.[0]?.content?.parts||[]).map(p=>p.text||"").join("").trim();
          if(text){if(state.model!==model){state.model=model;persist();}return text;}
          lastError="Gemini returned an empty answer.";break;
        }
        const status=response.status; lastError=data?.error?.message||`HTTP ${status}`;
        if(status===401||status===403) throw new Error(lastError);
        if(status===404) break;
        if([408,429,500,502,503,504].includes(status)){
          if(attempt<2){await new Promise(r=>setTimeout(r,(2**attempt)*1500+Math.floor(Math.random()*500)));continue;}
          break;
        }
        ...
        if(String(err.message||"").includes("API key")||String(err.message||"").includes("PERMISSION")||String(err.message||"").includes("permission")) throw err;
        lastError=err.message||lastError;break;
      }
    }
  }
  throw new Error("Gemini is temporarily busy or your quota is unavailable. ALVIN AI tried multiple models and automatic retries. Please wait a little and try again. Details: "+lastError);
}
